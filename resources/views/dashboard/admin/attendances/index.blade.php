...
<div class="card-body">
    ...
    <div class="text-center m-3">
        ...
        <a class="mr-2 font-weight-bold"
            href="{{ route('admin.attendances.index', ['year' => $link['year'], 'month' => $link['month']]) }}">
            {{ $link['fullName'] }}
        </a>
        @endif
        @endforeach
    </div>
    <form action="{{ route('admin.attendances.store', ['year' => $selectedYear, 'month' => $selectedMonth]) }}"
        method="post">
        @csrf
            ...
            <table class=" table-sm table-bordered table-striped table-hover datatable">
                <thead>
                    <tr>
                        ...
                        @for($i = 1; $i <= $daysInMonth; $i++) <th style="width: 5px">{{ $i }}</th>
                            @endfor
                    </tr>
                </thead>
                <tbody>
